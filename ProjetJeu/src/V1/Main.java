...
public class Test{
    ...
    static Joueur Game(final Joueurs j, final List<Joueur> j4) { //phase 1 du jeu
        ...
        if (select(j, t1) == -1) { //select permet au joueur d'avoir acces à des commandes specifiques
            return null; //sélection de l'option quitter la partie courante par le joueur d'ou arret
        }
        t1.deroulerPhaseJeu(j4); //lancement phase 1 du jeu
    ...
    static Joueur gameSuite(final Joueurs j, final List<Joueur> j4){ //phase 2 et phase 3 du jeu
        ...
        System.out.println("\nClassement actuel des Players :\n" + j);
        if(select(j, t2) == -1){ //select permet au joueur d'avoir acces à des commandes specifiques
            return null; //sélection de l'option quitter la partie courante par le joueur d'ou arret
        }
        t2.deroulerPhaseJeu(j4); //lancement phase 2 du jeu

        Phase3 t3 = new Phase3(); // début phase 2
        t3.selectionJoueur(j4); // classement score, avant dernier -> perdant et les autres restent gagnants
        System.out.println("\nClassement actuel des Players :\n" + j);
        if(select(j, t3) == -1){ //select permet au joueur d'avoir acces à des commandes specifiques
            return null; //sélection de l'option quitter la partie courante par le joueur d'ou arret
        }
        t3.deroulerPhaseJeu(j4); //lancement phase 3 du jeu
