...
public class Phase2 implements Phase {
    public void selectionJoueur(final Joueurs joueurs) {
        if (joueurs.getJoueurs().size() > 3) { //cas avec 4 joueurs -> le dernier joueur est perdant, pas qd le grand jeu d'ou size>3 (3j)
            joueurs.getJoueurs().get(0).updateEtat(Joueur.PERDU_P1); //le premier dans la liste est le perdant de la phase précedente
            System.out.println("\nPlayer" + joueurs.getJoueurs().get(0).getNum() + " tombe à la Phase 1 !");
        }
        for (int i = joueurs.getJoueurs().size()-3; i < joueurs.getJoueurs().size(); i++) { //les autres sont victorieux
            joueurs.getJoueurs().get(i).updateEtat(Joueur.VAINCQUEUR);
            System.out.println("Player" + joueurs.getJoueurs().get(i).getNum() + " accède à la Phase 2 !");
        }
    }
    ...
    public void deroulerPhaseJeu(final Joueurs joueurs){
        ...
        tmp = currThemes(tmp, joueurs.getJoueurs());
        int i = joueurs.getJoueurs().size() - 1;
        while (tmp.size() != 0) { //bouclage tant tant que les 6 thèmes n'ont pas été traités
            System.out.println("\nPlayer" + joueurs.getJoueurs().get(i).getNum() + " choississez le chiffre associé à un thème parmi :");
            ...
            currPhase(tmp.get(ans), joueurs, i); //on passe aux questions si le thème est bien parmi ceux de la liste
            ...
            if (joueurs.getJoueurs().size() == 4 && i == 0) {
                i = joueurs.getJoueurs().size() - 1; //on reboucle quand le i < 0 car ID est associé au joueur des positions 1 à 3 dans la liste
            }
            else if(joueurs.getJoueurs().size() == 3 && i == -1)
            {
                i = joueurs.getJoueurs().size() - 1;
            }
        }
        joueurs.getJoueurs().sort(Comparator.comparing(Joueur::getEtat).thenComparingInt(Joueur::getScore)
                .thenComparing(Comparator.comparingInt(Joueur::getTime).reversed()));
    ...
    public void currPhase(final String aleaTheme, final Joueurs joueurs, final int i) {
        ...
        System.out.println("\nLe thème choisi par Player" + joueurs.getJoueurs().get(i).getNum() + " est " + aleaTheme);
        ...
        for(int acc = 1; acc < 3; acc++) {
            ...
            {
                joueurs.getJoueurs().get(i).addScore(3); //incrémentation du score
                check = true; //permet d'afficher un message cohérent selon bonne ou mauvaise réponse du joueur
            }
            joueurs.getJoueurs().get(i).updateTime((int) (System.currentTimeMillis() - startTime)); //temps de réponse
